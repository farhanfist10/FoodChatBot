...
async def handle_request(request:Request):
    ...
    intent_handler_dict={
        ...
        'new.order':refresh_order,
    }
    return intent_handler_dict[intent](parameters,session_id)
    # if intent=="track.order-context:ongoing-order":
    #     return track_order(parameters)
def refresh_order(parameters:dict,session_id:str):
    if session_id in inprogress_orders:
        del inprogress_orders[session_id]
    return JSONResponse(content={
        "fulfillmentText": (
            "Starting new order. Specify food items and quantities. For example, you can say, "
            "\"I would like to order two pizzas and one mango lassi.\" Also, we have only the "
            "following items on our menu: Pav Bhaji, Chole Bhature, Pizza, Mango Lassi, Masala Dosa, "
...
def add_to_order(parameters:dict,session_id:str):
    ...
    if len(food_items) != len(quantities):
        fulfillment_text="Sorry I did't understand.Can you please specify food items and quantities clearly?"
    else:
